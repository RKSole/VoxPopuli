module.exports = {
    owner: 'Pablo Salinas',
    companyname: 'Zara',
    rating: '4.2',
    comments: 'Zara is a Spanish fast fashion (clothing and accessories) retailer based in Arteixo, Galicia (Spain). ',
    email: 'salinas@zara.com',
    nature: 'department stores', 
    image:url('https://www.foromarketing.com/wp-content/uploads/2016/04/zara.jpg'),
    telephoneNumber:6458854845,
    url: url('www.zara.com')
  },
  {
    owner: 'Javier Salero',
    companyname: 'Carrefour',
    rating: '3.5',
    comments: 'It is one of the largest hypermarket chains in the world with 12,300 self-service shops (including 1,528 hypermarkets) at the end of 2016.',
    email: 'salero@carreour.com',
    nature: 'department stores', 
    image:url('https://www.actuall.com/wp-content/uploads/2017/07/imagencabecera32854_63568949383041-696x696.jpg'),
    telephoneNumber:645842845,
    url: url('www.carrefour.com')
  },
  {
    owner: 'Manuel del Pozo',
    companyname: 'El Corte Ingles',
    rating: '3.2',
    comments: 'El Corte Inglés S.A., headquartered in Madrid, is the biggest department store group in Europe and ranks fourth worldwide.',
    email: 'pozo@eci.com',
    nature: 'department stores',
    image:url('https://cdn.elgrupoinformatico.com/img/w720/Noticias/2016/01/el-corte-ingles-logo-220116.jpg')
  },
  {
    owner: 'Francisco Camacho',
    companyname: 'Cines Kinepolis',
    rating: '4.4',
    comments: 'The Kinepolis Group is a Belgian cinema chain formed in 1997 as a result of the merger of two family cinema groups, Bert and Claeys, and has been listed on the stock exchange since 1998.',
    email: 'camacho@kine.com',
    nature: 'entertainment',
    image:url('https://www.granadaon.com/wp-content/uploads/2017/04/kinepolis-cines-granada-1-1.jpg')
  },
  {
    owner: 'Gloria García',
    companyname: 'Hospital Gregorio Marañón',
    rating: '4',
    comments: 'Gregorio Marañón Hospital is a Spanish health institution located as one of the most advanced public health centers.',
    email: 'garcia@HG.com',
    nature: 'health centers',
    image:url('http://farmacovigilancia.tv/blog/wp-content/uploads/2010/05/SALUD_MADRID.jpg')
  },
  {
    owner: 'Susana Vadillo',
    companyname: 'Game',
    rating: '6',
    comments: 'GAME is a british company dedicated to the sale, rental and loans of video games',
    email: 'Vadillo@Game.com',
    nature: 'department stores',
    image:url('http://www.nintenderos.com/wp-content/uploads/2015/10/GAME-logotipo.jpg')
  },
  {
    owner: 'Giorgio',
    companyname: 'GoikoGrill',
    rating: '4.5',
    comments: 'Goiko Grill was born in a small space with room for only 30 seats. It was a dream of Andoni’s, a medical doctor who originally came from Caracas to work as a resident medical officer at Hospital de La Paz, but ended up ditching the white robe for the apron.',
    email: 'Girodio@Goiko.com',
    nature: 'hospitality',
    image:url('http://www.negociofranquicia.es/wordpress/wp-content/uploads/2017/05/goiko-grill-negocio-franquicia.jpg')
  },
  {
    owner: 'Marc Pomar',
    companyname: 'Universidad Complutense de Madrid',
    rating: '3',
    comments: 'The UCM is a university with a long history and broad social recognition, located in Madrid.',
    email: 'Pomar@ucm.com',
    nature: 'educational institutions',
    image:url('https://www.ucm.es/data/cont/media/www/3/Marca%20UCM%20logo%20negro.jpg')
  },
  {
    owner: 'Victor Rodriguez',
    companyname: 'IronHack Madrid',
    rating: '3.6',
    comments: 'Preparing the next generation of digital creators',
    email: 'Rodriguez@IM.com',
    nature: 'educational institutions',
    image:url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ8sYw01BogzH18tKo0fclwyKda9flRGD02yhVdyGjRKqaMf5LCCg')
  },
  {
    owner: 'Beltran S.',
    companyname: 'Mango',
    rating: '3.8',
    comments: 'MANGO was founded in 1984 and is today one of the leading fashion groups in the world.Based in its city of origin, Barcelona',
    email: 's@mango.com',
    nature: 'department stores',
    image:url('https://ballzbeatz.com/wp-content/uploads/2018/01/Mango-Logo-Decal-Sticker.jpg')
  },
  {
    owner: 'Claudia Losada',
    companyname: 'Embajada Española en Londres',
    rating: '3.7',
    comments: 'Spanish Embassy in London was created long time ago to help every spanish around this amazing city',
    email: 'losada@Se.com',
    nature: 'government institutions',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/8/85/Escudo_de_Espa%C3%B1a_%28mazonado%29.svg/200px-Escudo_de_Espa%C3%B1a_%28mazonado%29.svg.png')
  },
  {
    owner: 'Antonella',
    companyname: 'Parador Nacional Alcalá',
    rating: '4.7',
    comments: 'A parador in Spain and other Spanish-speaking countries, is a kind of luxury hotel, usually located in a converted historic building such as a monastery or castle. Parar means to stop, halt or stay. Contents.',
    email: 'anto@parador.com',
    nature: 'hospitality',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/b/be/Logotipo_Paradores.jpg/1200px-Logotipo_Paradores.jpg')
  },
  {
    owner: 'Carlos Fernandez',
    companyname: 'Eurostars Hotels',
    rating: '3.6',
    comments: 'Eurostars Hotels is a hotel chain belonging to the Hotusa Group with a portfolio of 75 establishments distributed in 14 European and American destinations',
    email: 'fernandez@euro.com',
    nature: 'hospitality',
    image:url('https://pbs.twimg.com/profile_images/793746608118726656/feMNEUt0_400x400.jpg')
  },
  {
    owner: 'Miriam Lozano',
    companyname: 'Casino de Torrelodones',
    rating: '3.4',
    comments: 'The Casino Gran Madrid is a gambling casino located at kilometer 29 of the A-6, in the municipality of Torrelodones Spain, in the area known as Los Llanos. It is the first casino in Spain obout number of visits.',
    email: 'lozano@casino.com',
    nature: 'entertainment',
    image:url('https://s3.eu-central-1.amazonaws.com/images.jacksonlive.es/upload/spots/high/1501171297.jpg')
  },
  {
    owner: 'Isabel Carrascosa',
    companyname: 'Hotel Mayorazgo',
    rating: '4',
    comments: "The Mayorazgo hotel is a 4 star hotel very close to Madrid's Gran Vía and Plaza de España, which makes it an option for the commercial and business traveler",
    email: 'carras@mayor.com',
    nature: 'hospitality',
    image:url('http://www.hotelbooqi.com/wp-content/uploads/2017/08/Mayorazgo.png')
  },
  {
    owner: 'Francisco Llanos',
    companyname: 'Estadio Vicente Calderón',
    rating: '94.8',
    comments: 'Stadium of Atlético de Madrid FC',
    email: 'llanos@atletico.com',
    nature: 'sport facility',
    image:url('https://images-na.ssl-images-amazon.com/images/I/714hhiMYyzL._SX425_.jpg')
  },
  {
    owner: 'Fermin del Prado',
    companyname: 'Estadio Santiago Bernabeu',
    rating: '3',
    comments: 'Stadium of Real Madrid FC',
    email: 'prado@bernabeu.com',
    nature: 'sport facility',
    image:url('https://pbs.twimg.com/profile_images/855073776215654400/oGEOJ2JU.jpg')
  },
  {
    owner: 'Michael Lans',
    companyname: 'Amazon',
    rating: '4.2',
    comments: 'Amazon is an American electronic commerce and cloud computing company based in Seattle, Washington that was founded by Jeff Bezos on July 5, 1994.',
    email: 'lans@amazon.com',
    nature: 'e-commerce',
    image:url('http://g-ec2.images-amazon.com/images/G/01/social/api-share/amazon_logo_500500.png')
  },
  {
    owner: 'Kevin Ranth',
    companyname: 'Google',
    rating: '4.7',
    comments: 'Google Inc., American search engine company, founded in 1998 by Sergey Brin and Larry Page that is a subsidiary of the holding company Alphabet Inc.',
    email: 'ranth@google.com',
    nature: 'e-commerce',
    image:url('http://jessiecervantes.com/wp-content/uploads/2018/04/Google.jpg ')
  },
  {
    owner: 'Tomás Gonzalez',
    companyname: 'PcComponentes',
    rating: '4.1',
    comments: 'electronic commerce dedicated to sale informatical hardware',
    email: 'gonza@pcomp.com',
    nature: 'e-commerce',
    image:url('https://3.bp.blogspot.com/-qxm_6NjgHJs/UXUJ7aThRgI/AAAAAAAAABU/crV5eJUczf8/s1600/PcComponentes.gif')
  },
  {
    owner: 'Huei Lin',
    companyname: 'Alibaba',
    rating: '2.5',
    comments: 'Alibaba Group Holding Limited, incorporated on June 28, 1999, is a holding company. Through its subsidiaries, the Company is principally engaged in online and mobile commerce',
    email: 'lin@ali.com',
    nature: 'e-commerce',
    image:url('http://ticker.tv/news/wp-content/uploads/2018/01/BABA.png')
  },
  {
    owner: 'Javier Rodrigo',
    companyname: 'Bowling Chamartin',
    rating: '4',
    comments: 'The best bowling around city',
    email: 'rodri@bowl.com',
    nature: 'entertainment',
    image:url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOnC7nMAObpKFBHk9Prbwm7Zm4UeRlh_uNjlAPOExdM8fQw3Kn')
  },
  {
    owner: 'Abdallah Aberouch',
    companyname: 'BBVA',
    rating: '2.6',
    comments: 'BBVA is a multinational Spanish banking group. It was formed from a merger of Banco Bilbao Vizcaya and Argentaria in 1999, and is the second largest bank in Spain.',
    email: 'abe@bbva.com',
    nature: 'finance',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/7/70/Logotipo_de_BBVA.svg/1280px-Logotipo_de_BBVA.svg.png')
  },
  {
    owner: 'Jaime risueña',
    companyname: 'Plaza de Toros de las Ventas',
    rating: '4.3',
    comments: "world's bullfight temple",
    email: 'risu@ventas.com',
    nature: 'entertainment',
    image:url('http://www.abc.es/media/cultura/2017/06/22/plaza%20ventas-kKa--620x349@abc.jpg')
  },
  {
    owner: 'Frank Warthbenger',
    companyname: 'Tomorrowland',
    rating: '4.2',
    comments: 'The most famous electronic music festival',
    email: 'wart@tomo.com',
    nature: 'entertainment',
    image:url('http://www.tomorrowland.com/global/img/og.jpg')
  },
  {
    owner: 'Oscar Millan',
    companyname: 'Madrid Rio',
    rating: '7',
    comments: 'mucis festival based in Madrid',
    email: 'millan@rio.com',
    nature: 'entertainment',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/b/b4/Rock_in_Rio_-_Madrid_2012.jpg/1200px-Rock_in_Rio_-_Madrid_2012.jpg')
  },
  {
    owner: 'Fernando Meyado',
    companyname: 'Sala "La Riviera"',
    rating: '6.9',
    comments: 'Located on the banks of the Manzanares River, the Riviera room has offered more than half a century to the locals an offer of shows hardly comparable to that of other venues in the capital. With an area of ​​1,500 square meters and a capacity of 2,500 spectators',
    email: 'meya@riviera.com',
    nature: 'entertainment',
    image:url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjCpZ_DsXI2ZfYHdZdweDcPW5JiAQthBr5Q6WlRFxY2OcqCcJj7A')
  },
  {
    owner: 'Lorena Campos',
    companyname: 'Red de metro (Comunidad de Madrid)',
    rating: '3.9',
    comments: "underground's network of the Spanish capital",
    email: 'campos@mdm.com',
    nature: 'transport system',
    image:url('http://entreparentesis.org/wp-content/uploads/FOTO-Metro-Madrid.png')
  },
  {
    owner: 'Esther yoyo',
    companyname: 'Red de autobuses (Comunidad de Madrid)',
    rating: '3.8',
    comments: "buse's network of the Spanish capital",
    email: 'yoyo@bus.com',
    nature: 'transport system',
    image:url('http://www.iconmm.com/wp-content/uploads/2017/10/transit_emt.jpg')
  },
  {
    owner: 'Penelope Marco',
    companyname: 'Lidl España',
    rating: '4',
    comments: 'Lidl Stiftung & Co. KG, better known by its trademark Lidl, is a German discount supermarket chain',
    email: 'Marco@lidl.com',
    nature: 'department stores',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/9/91/Lidl-Logo.svg/200px-Lidl-Logo.svg.png')
  },
  {
    owner: 'Manolo Pedroño',
    companyname: 'Spa Balneario Madrid Sol y Agua',
    rating: '3.1',
    comments: 'This Spa is a hymn to nature, intriguing to the senses yet calming to the mind.',
    email: 'pedro@Spa.com',
    nature: 'wellness industry',
    image:url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8_d7IY_CXjaXX-dzd12hE5loN6YORg7DWcCeYMlMMn5Uop5X3')
  },
  {
    owner: 'Ignacio chamberi',
    companyname: 'Clínicas Dorsia',
    rating: '2.9',
    comments: 'We use medicine is an inclusive term for specialties that focus on improving cosmetic appearance through the treatment of conditions including scars, skin laxity, wrinkles, moles, liver spots, excess fat, cellulite, unwanted hair, skin discoloration...',
    email: 'chamberi@dermo.com',
    nature: 'wellness industry',
    image:url('http://www.franquiciator.es/wp-content/uploads/2013/01/dorsia.jpg')
  },
  {
    owner: 'Alejandro de la cruz',
    companyname: "Ruber International Hospital",
    rating: '4.1',
    comments: 'We use medicine is an inclusive term for specialties that focus on improving cosmetic appearance through the treatment of conditions including scars, skin laxity, wrinkles, moles, liver spots, excess fat, cellulite, unwanted hair, skin discoloration...',
    email: 'cruz@clinic.com',
    nature: 'wellness industry',
    image:url('http://padres20.org/wp-content/uploads/2016/01/HRI.png')
  },
  {
    owner: 'Claudio Brasero',
    companyname: 'Teatro Barceló',
    rating: '4.3',
    comments: ' four spaces local with bars, lights and dance floor with DJ, located in the old Barceló Theater.',
    email: 'brasero@barcelo.com',
    nature: 'entertainment',
    image:url('http://media.tilllate.es/eventflyers/fcef3fd8ea2f0c0fb56ebc5dc2a2e128.jpeg')
  },
  {
    owner: 'Pablo Merino',
    companyname: 'Mercado de SanMiguel',
    rating: '4.7',
    comments: 'The Market of San Miguel is a covered market located in Madrid, Spain. Originally built in 1916, it was purchased by private investors in 2003 who renovated the iron structure and reopened it in 2009.',
    email: 'merino@sanmiguel.com',
    nature: 'hospitality',
    image:url('http://1.bp.blogspot.com/-6OLl1Kk0FNM/UZS4GpmrAYI/AAAAAAAAIUk/OwpQd2QdmI8/s1600/ENTA.jpg')
  },
  {
    owner: 'Aleix Revuelto',
    companyname: '100 montaditos',
    rating: '3.8',
    comments: 'One of the most famous bars in Madrid, come with your friends!.',
    email: 'revuelto@cañas.com',
    nature: 'hospitality',
    image:url('https://colombia.100montaditos.com/wp-content/themes/100montaditos/images/conocenos.jpg')
  },
  {
    owner: 'Alfredo Sofias',
    companyname: 'Sol Meliá',
    rating: '4.2',
    comments: 'Founded in 1956 in Palma de Mallorca, Spain, Meliá Hotels International is one of the largest hotel companies in the world and the largest hotel chain in Spain in both resort and city hotels. The company currently operates more than 370 hotels in 43 countries',
    email: 'sofias@melia.com',
    nature: 'hospitality',
    image:url('https://www.brandsofspain.com/wp-content/uploads/2011/07/WEB-MHI-centrado-750x430.jpg')
  },
  {
    owner: 'Marta Soledad',
    companyname: 'NH Hotels',
    rating: '4.8',
    comments: 'NH Hotel Group is a Spanish-based hotel chain headquartered in Madrid. NH offers moderately priced and modernly-furnished hotel rooms and lobbies, located primarily in Europe and Latin America.',
    email: 'sole@nh.com',
    nature: 'hospitality',
    image:url('https://pbs.twimg.com/profile_images/920325886536224768/Hj07B7XQ_400x400.jpg')
  },
  {
    owner: 'Pedro Trolero',
    companyname: 'Navas & Cusi Abogados Madrid',
    rating: '3.7',
    comments: 'Law firms in the city, your problem concern us',
    email: 'trol@navasycusi.com',
    nature: 'SMEs',
    image: url('https://s3-media2.fl.yelpcdn.com/bphoto/HnWtCq1U_OO6auu9C-3g7g/o.jpg')
  },
  {
    owner: 'Nessi Carles',
    companyname: 'Clinica dental Odental',
    rating: '3.1',
    comments: 'dental clinic',
    email: 'carles@odental.com',
    nature: 'health center',
    image:url('https://pbs.twimg.com/profile_images/748004862516932608/PEnJ_OZv_400x400.jpg')
  },
  {
    owner: 'Franchesca Ferrari',
    companyname: 'Independance',
    rating: '4.2',
    comments: 'independance club was born on June 16 of 2006 and soon became a pioneer and reference in the scene of indie clubs in the capital.',
    email: 'ferrari@ind.com',
    nature: 'entertainment',
    image:url('https://pbs.twimg.com/profile_images/137865563/1226578972373_f_400x400.jpg')
  },
  {
    owner: 'Braulio Parreño',
    companyname: 'Ikea',
    rating: '4.7',
    comments: 'IKEA is a Swedish-founded Dutch-based multinational group, that designs and sells ready-to-assemble furniture, kitchen appliances and home accessories.',
    email: 'parreño@ikea.com',
    nature: 'large corporations',
    image:url('https://logotipogratis.com/blog/wp-content/uploads/2017/10/1665397_orig.jpg')
  },
  {
    owner: 'Brian McBrian',
    companyname: 'MacDonalds',
    rating: '4.3',
    comments: "McDonald's is the leading global foodservice retailer with over 36000 restaurants in more than 100 countries around the world.",
    email: 'mcbrian@mc.com',
    nature: 'hospitality',
    image:url('https://franciscotorreblanca.es/wp-content/uploads/2018/03/historia-origen-marcas-mcdonalds.jpg')
  },
  {
    owner: 'Royal Burguer',
    companyname: 'Burguer king',
    rating: '4.2',
    comments: 'Burger King Corporation operates a chain of fast food hamburger restaurants worldwide.',
    email: 'burguer@king.com',
    nature: 'hospitality',
    image:url('https://static.burgerkingencasa.es/bkhome/Images/BKlogo.png')
  },
  {
    owner: 'Pol Alas',
    companyname: 'KFC',
    rating: '4',
    comments: 'KFC popularized chicken in the fast food industry, diversifying the market by challenging the established dominance of the hamburger.',
    email: 'alas@kfc.com',
    nature: 'hospitality',
    image:url('https://pbs.twimg.com/profile_images/803220446565040128/yCZ-z_aB_400x400.jpg')
  },
  {
    owner: 'Mateo Arrivas',
    companyname: 'Parque de atracciones de Madrid',
    rating: '3.9',
    comments: 'First amusement park of Madrid',
    email: 'arrivas@pam.com',
    nature: 'entertainment',
    image:url('https://esferize.com/wp-content/uploads/2015/04/parque.png')
  },
  {
    owner: 'Jose del Río',
    companyname: 'Tesorería de hacienda',
    rating: '2.7',
    comments: 'national treasury institution of Spain',
    email: 'rio@hacienda.com',
    nature: 'government institutions',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/7/7a/Agencia_Tributaria.svg/1200px-Agencia_Tributaria.svg.png')
  },
  {
    owner: 'Mariano Fuentes',
    companyname: 'Oficinas de empleo de la Comunidad de Madrid',
    rating: '2.5',
    comments: 'national employment office of Spain',
    email: 'fuentes@empleo.com',
    nature: 'government institutions',
    image:url('http://gestiona.madrid.org/CTAC_CITA/img/icm/logoempleo.png')
  },
  {
    owner: 'Luna Bellido',
    companyname: 'Police Department',
    rating: '3.1',
    comments: 'National police corps of Spain',
    email: 'bellido@pn.com',
    nature: 'government institutions',
    image:url('https://teleoposiciones.es/wp-content/uploads/2018/01/policia-nacional-1024x768.png')
  },
  {
    owner: 'Esther rancho',
    companyname: 'Mercadona',
    rating: '4.1',
    comments: 'Mercadona, S.A. owns and operates supermarkets in Spain. It offers food and beverages, cleaning products, personal hygiene products, and pet care products.',
    email: 'rancho@mercadona.com',
    nature: 'department stores',
    image:url('https://t1.uc.ltmcdn.com/images/9/2/5/img_como_enviar_el_curriculum_para_trabajar_en_mercadona_47529_orig.jpg')
  },
  {
    owner: 'Nacho Casado',
    companyname: 'El Brillante',
    rating: '4.8',
    comments: "famous Squid's restaurant",
    email: 'casado@brillante.com',
    nature: 'hospitality',
    image:url('http://www.barelbrillante.es/wp-content/uploads/2013/07/LOGO-BRILLANTE-02.png')
  },
  {
    owner: 'Yago Fritos',
    companyname: 'Museo del Pan Gallego',
    rating: '4.3',
    comments: 'ypical bakery with traditional Galician products',
    email: 'fritos@gallego.com',
    nature: 'hospitality',
    image:url('https://media.timeout.com/images/102088553/630/472/image.jpg')
  },
  {
    owner: 'Gabriel Corrales',
    companyname: 'Media Markt',
    rating: '4.8',
    comments: 'The Group operates more than 1000 stores and offers online shopping in 15 countries in Europe. In the business year 2015/16, MediaMarktSaturn generated revenues of 22 billion euros. ',
    email: 'corrales@mm.com',
    nature: 'department stores',
    image:url('https://img.europapress.es/fotoweb/fotonoticia_20171121180015_640.jpg')

  },
  {
    owner: 'Alvaro pastilla',
    companyname: 'Aussieyoutoo',
    rating: '3.8',
    comments: 'Educational agency',
    email: 'pastilla@aussie.com',
    nature: 'SMEs',
    image:url('http://xarxanet.org/sites/default/files/aussiyou.png')
  },
  {
    owner: 'James Smith',
    companyname: 'Uber',
    rating: '3.6',
    comments: 'Uber Technologies, Inc. provides e-commerce services for car hire. The Company offers a website and develops application that allows users to request a car for hire from any mobile device text message.',
    email: 'smith@uber.com',
    nature: 'transport system',
    image:url('https://www.numeroservicioalcliente.com/wp-content/uploads/2017/06/servicio-al-cliente-uber.png')
  },
  {
    owner: 'Marcos Señuelo',
    companyname: 'Aeropuerto Madrid Barajas-Adolfo Suarez',
    rating: '4.6',
    comments: 'Madrid-Barajas Airport (code IATA: MAD, code ICAO: LEMD) is located on the northeast of Madrid, Barajas district, only 12 kilometers from Madrid city centre. The airport changed its name from Madrid-Barajas to Adolfo Suárez Madrid-Barajas in 2014. ',
    email: 'señu@barajas.com',
    nature: 'transport system',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/d/df/Barajas_Airport_%28Madrid%29_%284684560779%29.jpg/1200px-Barajas_Airport_%28Madrid%29_%284684560779%29.jpg')
  },
  {
    owner: 'Alicia Sevillano',
    companyname: 'Airsoft Pozuelo',
    rating: '4',
    comments: ' The airsoft is a playful-sporting strategy activity based on military simulation,',
    email: 'sevi@airsoft.com',
    nature: 'sport facility',
    image:url('https://www.estrelladigital.es/media/estrelladigital/images/2017/07/06/2017070611100190122.jpg')
  },
  {
    owner: 'Virginia Palos',
    companyname: 'Carlos Sainz Center',
    rating: '3.5',
    comments: 'Spend a fun day, celebrate your birthday in an original way or a different company event. Get to know our Karting circuits in Madrid!',
    email: 'palos@csc.com',
    nature: 'entertainment',
    image:url('https://pbs.twimg.com/profile_images/935548407212625925/tGGrQm7y_400x400.jpg')
  },
  {
    owner: 'Victor Pozuelo',
    companyname: 'Virgin Active',
    rating: '3.4',
    comments: 'Welcome to Virgin Active Health Clubs, Gyms & Spas',
    email: 'Pozu@holidays.com',
    nature: 'sport facility',
    image:url('https://www.vectorlogo.es/wp-content/uploads/2016/12/logo-vector-virgin-active.jpg')
  },
  {
    owner: 'David Mile',
    companyname: 'Fnac',
    rating: '3.7',
    comments: "The FNAC chain of retail stores is France's leading purveyor of what the company refers to as cultural goods.",
    email: 'mile@fnac.com',
    nature: 'department stores',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/2/2e/Fnac_Logo.svg/1200px-Fnac_Logo.svg.png')
  },
  {
    owner: 'Rosario pueblas',
    companyname: 'Pans&Company',
    rating: '3.4',
    comments: ' Pans & Company is a fast food company in the sandwich industry. The brand offers snacks in fast service. It belongs to the restoration group The Eat Out Group,',
    email: 'pueblas@p&c.com',
    nature: 'hospitality',
    image:url('http://www.cenabarato.com/wp-content/uploads/2014/09/logo-pans-company-450x344.jpg')
  },
  {
    owner: 'Luis Avellaneda',
    companyname: 'Vodafone',
    rating: '2',
    comments: 'Vodafone Group Plc is a telecommunications company. It provides a range of services including voice, messaging, data, and fixed communications',
    email: 'ave@vodafon.com',
    nature: 'large corporations',
    image:url('https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Vodafone_2017.svg/1200px-Vodafone_2017.svg.png')
  },
  {
    owner: 'Santiago Jerez',
    companyname: 'Telefónica',
    rating: '2.3',
    comments: 'Telefónica, S.A. is a Spanish multinational broadband and telecommunications provider with operations in Europe, Asia, and North, Central and South America. ',
    email: 'jerez@telefonica.com',
    nature: 'large corporations',
    image:url('http://s03.s3c.es/imag/_v0/770x420/f/1/b/telefonica-logo-nuevo.jpg')
  },
  {
    owner: 'Jesus Carreras',
    companyname: 'Jazztel',
    rating: '2.4',
    comments: 'Jazztel is a telecom that operates in Spain offering triple play services (telephone, Internet and television). Jazztel was founded in 1998 by Martin Varsavsky. Jazztel is a Spanish company that is owned by a British company called Jazztel p.l.c. It was incorporated under the laws of England and Wales in July 1998.',
    email: 'carreras@jazztel.com',
    nature: 'large corporations',
    image:url('https://css.adslayuda.com/wp-content/uploads/2018/01/portada_Jazztel.jpg')
  },
  {
    owner: 'Raúl Irisa',
    companyname: 'Autoescuela Gala',
    rating: '3.2',
    comments: 'we are teaching since 1960, best driving school',
    email: 'Iris@dali.com',
    nature: 'educational institutions',
    image:url('https://i.ytimg.com/vi/FS6QE5oV6vc/maxresdefault.jpg')
  };